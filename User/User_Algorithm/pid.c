...
void PID_Regular_Cacl(PID_Regular_t* PID_Regular)
{
	...
	Output	=	PID_Regular -> Kp * Error 
			+	PID_Regular -> Ki * Integrate
			-	PID_Regular -> Kd * Delta;
...
void PID_Increment_Calc(PID_Increment_t* PID_Increment)
{
	...
	DeltaOutput	=	PID_Increment -> Kp * (PID_Increment -> Error - PID_Increment -> LastError)
				+	PID_Increment -> Ki * PID_Increment -> Error
				+	PID_Increment -> Kd * (PID_Increment -> Error - 2 * PID_Increment -> LastError
...
{
	PID_Increment -> Ref = 0;
	PID_Increment -> Error = 0;
	PID_Increment -> Feedback = 0;
	PID_Increment -> IntError = 0;
	PID_Increment -> LastError = 0;
	PID_Increment -> PrevError = 0;
}
